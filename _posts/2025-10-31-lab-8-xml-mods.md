...
4. Data addition/modification (based on validation). This step builds on the previous validation question. Once you identify the local call numbers, then ensure they are properly formatted (validation), add a `type` attribute to indicate their source authority.
5. Save the records that you updated in the previous question to a new file, which is valid XML (well-formed, has namespace declaration, and appropriate qualifiers for MODS and any other schemas used in the output).
